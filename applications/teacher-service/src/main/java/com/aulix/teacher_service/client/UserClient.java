...
import com.aulix.teacher_service.exception.UserProvisioningException;
...
public class UserClient {
        ...
        ApiResponse<UserResponse> response = restClient.post()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
                    if (res.getStatusCode().value() == 409) {
                        throw new DuplicateTeacherException("email", request.email());
                    }
     ...
     * resolve the set of userIds a teacher free-text search should also match against.
     */
    ...
    /**
     * Batch-fetches users by id, used to enrich a page of Teacher with their user-domain data
     * (name, email) in a single round trip instead of one call per teacher.
     */
    ...
    public UserResponse updateUserByUserId(UUID id, UpdateTeacherRequest request) {
