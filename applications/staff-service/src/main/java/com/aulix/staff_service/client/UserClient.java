package com.aulix.staff_service.client;
...
import com.aulix.staff_service.exception.UserProvisioningException;
...
public class UserClient {
        ...
        ApiResponse<UserResponse> response = restClient.post()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
                    if (res.getStatusCode().value() == 409) {
                        throw new DuplicateStaffException("email", request.email());
                    }
     ...
     * resolve the set of userIds a staff free-text search should also match against.
     */
    ...
    /**
     * Batch-fetches users by id, used to enrich a page of Staff with their user-domain data
     * (name, email) in a single round trip instead of one call per staff member.
     */
    ...
    public UserResponse updateUserByUserId(UUID id, UpdateStaffRequest request) {
